        ...
        <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-[#777] uppercase tracking-widest">
          {room.bedType && (
            <span className="flex items-center gap-1.5">
              <BedDouble size={12} className="text-[#651D4C]" />
              {room.bedType}
            </span>
          )}
          {room.size && (
            <span className="flex items-center gap-1.5">
              <Maximize2 size={12} className="text-[#651D4C]" />
              {room.size}
            </span>
          )}
          {(room.maxGuests || room.capacity) && (
            <span className="flex items-center gap-1.5">
              <Users size={12} className="text-[#651D4C]" />
              Max {room.maxGuests || room.capacity} guests
            </span>
          )}
        </div>
      ...
      <section className="bg-white py-16 text-center lg:py-20">
        ...
        >
          ...
          <p className="mx-auto mt-6 max-w-xl px-6 text-[14px] leading-relaxed text-[#555555]">
            Choose from our selection of thoughtfully appointed rooms and
            suites, each crafted to deliver comfort, style, and a sense of
            island sanctuary on Mactan Island, Cebu.
          </p>
        </motion.div>
      </section>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24">
              <p className="text-red-500 font-medium">
                Unable to load rooms. Please ensure the server is running on
                port 5001.
              </p>
            </div>
          ) : rooms.length === 0 ? (
            <p className="text-center py-24 text-[#555]">
